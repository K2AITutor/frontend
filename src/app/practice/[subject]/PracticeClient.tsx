...
export default function PracticeClient({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = (await submitAnswer(String(question.id), answer)) as MarkingResult;
...
type MarkingResult = {
    ...
    // new marking-engine fields (may or may not be present depending on backend response)
    score?: number | null;
    maxScore?: number | null;
    errorTags?: string[] | null;
    aiExplanation?: string | null;
...
const ERROR_TAG_TUTOR_TIPS: Record<string, string> = {
    "CALC_ERR.PRODUCT_RULE": "Tip: For y = u·v, use (uv)' = u'v + uv'.",
    "CALC_ERR.CHAIN_RULE": "Tip: Differentiate the outer function, then multiply by the inner derivative.",
    "CALC_ERR.QUOTIENT_RULE": "Tip: For y = u/v, use (u/v)' = (u'v − uv')/v².",
    "CALC_ERR.TRIG_DERIVATIVE": "Tip: Remember d/dx(cos x) = −sin x and d/dx(sin x) = cos x.",
    "META_ERR.ENDPOINT_INCLUSION":
        "Tip: Use [ ] when the endpoint is included, and ( ) when it is not.",
    "INT_ERR.MISSING_CONSTANT": "Tip: If it’s an indefinite integral, always include + C.",
};
...
export default function PracticeClient({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // submitAnswer expects string id in your code; keep that contract
            const res = (await submitAnswer(String(question.id), answer)) as MarkingResult;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ✅ RESULT PANEL (enhanced with marking output) }
            {result && (
                <div
                ...
                >
                    ...
                    {/* ✅ Examiner-style feedback from error tags }
                    {!result.correct && errorTags.length > 0 && (
                        <div className="mt-3 text-slate-200">
                            <p className="font-semibold mb-2">Examiner feedback</p>
                            <ul className="list-disc ml-6 space-y-1">
                                {errorTags.map((tag) => (
