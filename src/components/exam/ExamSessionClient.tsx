...
export default function ExamSessionClient(props: {
    ...
    subject: string; // keep for compatibility, not used for fetching
    examKey: string;
    ...
    // ✅ No fetching. Single source of truth: props.initialQuestions
    ...
    const qLabel = question.questionNumber?.trim()
        ? `Question ${question.questionNumber}`
        : `Question ${currentIndex + 1}`;

    const isFlagged = Boolean(flagged[String(question.id)]);
                ...
                <div className="glass p-3 sticky top-4">
                    <div className="flex items-center justify-between mb-3">
                        <div className="text-sm text-slate-300">
                            Exam PDF
                            {question.pdfPage ? ` • page ${question.pdfPage}` : ""}
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 text-slate-300">
                        <div className="text-sm">
                            {qLabel} <span className="text-slate-500">•</span>{" "}
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={toggleFlag}
                                className={`px-3 py-2 rounded-lg text-sm ${isFlagged
                                ...
                                {showDebug && (
                                    <div className="mt-2 text-xs text-slate-200 space-y-2">
                                        <div>
                                            ...
                                            <div>
                                                correct:{" "}
                                                <span className="font-semibold">{String(normalizedCorrect)}</span>, score:{" "}
                                ...
                                <div className="mt-3 text-slate-200">
                                    ...
                                    {errorTags.length > 0 ? (
                                            ...
                                            {errorTags.map((tag) => (
                                                <li key={tag}>
                                                    ...
                                                    {ERROR_TAG_TUTOR_TIPS[tag] && (
                                                        <div className="text-slate-400 text-sm mt-1">{ERROR_TAG_TUTOR_TIPS[tag]}</div>
                                                    )}
                                                </li>
                                            ))}
                                        </ul>
                                        ...
                                        <div className="text-slate-300">
                                            <p>
                                                No specific examiner tag was returned for this answer. This usually means the submission was
                                                invalid/unparseable, or this question doesn’t have tagged common wrong answers yet.
                                            </p>
                                            <p className="text-slate-400 text-sm mt-2">
                                                Tip: Rewrite using standard notation (e.g. use * for multiply, brackets for intervals, exact
                                                values like 1/3 or √5).
                                            </p>
                                        </div>
                                    )}
                                </div>
