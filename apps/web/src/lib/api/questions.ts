"use client";

import { useMutation, useQuery } from "@tanstack/react-query";
import { getSession, useSession } from "next-auth/react";
import { apiGet, apiPost, apiPut } from "@/lib/apiClient";
import { PATH } from "@aitutor/shared";
import type {
  QuestionDraftDTO,
  CreateQuestionDraftPayload,
  UpdateQuestionDraftPayload,
} from "@aitutor/shared";

export type {
  QuestionDraftDTO,
  CreateQuestionDraftPayload,
  UpdateQuestionDraftPayload,
};

async function getAccessToken() {
  const session = await getSession();
  return (session?.user as any)?.accessToken as string | undefined;
}

export function usePracticeQuestions(topic: string) {
  return useQuery({
    queryKey: ["practiceQuestions", topic],
    queryFn: async () => {
      return apiGet<any>(`${PATH.questions.practice}?topic=${encodeURIComponent(topic)}`);
    },
    enabled: !!topic,
  });
}

export function useSubmitAnswer() {
  return useMutation({
    mutationFn: async ({ questionId, answer }: { questionId: string; answer: string }) => {
      return apiPost<any>(PATH.questions.submit, { questionId, answer });
    },
  });
}

export function useContributorQuestionDrafts() {
  const { data: session } = useSession();
  const token = (session?.user as any)?.accessToken as string | undefined;

  return useQuery({
    queryKey: ["contributorQuestionDrafts", token],
    queryFn: () => apiGet<QuestionDraftDTO[]>(PATH.questions.draftsMine, token),
    enabled: !!token,
    staleTime: 30_000,
  });
}

export async function createQuestionDraft(payload: CreateQuestionDraftPayload) {
  const token = await getAccessToken();
  return apiPost<QuestionDraftDTO>(PATH.questions.drafts, payload, token);
}

export async function updateQuestionDraft(
  questionId: number | string,
  payload: UpdateQuestionDraftPayload
) {
  const token = await getAccessToken();
  return apiPut<QuestionDraftDTO>(PATH.questions.draftById(Number(questionId)), payload, token);
}

export async function submitQuestionDraftForReview(questionId: number | string) {
  const token = await getAccessToken();
  return apiPost<{ id: number; status: string; updatedAt: string }>(
    PATH.questions.submitReview(Number(questionId)),
    {},
    token
  );
}
